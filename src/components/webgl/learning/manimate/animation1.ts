...
export default class Animation1 extends AnimationBase {
	...
	async animate2() {
		...
		for(let i = 0; i < this.planes.length; i++) {
			...
			(<MyMat>this.planes[i].material).uniforms.u_appear.value = 1;
			...
			const del: number = Easing.easeOutCubic(i/this.planes.length)*this.planes.length * 0.4 + cnt
			cnt+= 0.1
			this.animation[i] = gsap.timeline().delay(del)
	...
	async animate3() {
		...
		for(let i = 0; i < this.planes.length; i++) {
			...
			(<MyMat>this.planes[i].material).uniforms.u_appear.value = 1;
			if(this.animation[i] != null) this.animation[i].kill()
			const del: number = Easing.easeOutExpo(i/this.planes.length)*this.planes.length * 0.15 + cnt
			cnt+= 0.07
			this.animation[i] = gsap.timeline().delay(del)
